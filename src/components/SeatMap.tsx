...
import { User, Armchair, Accessibility, DoorOpen, Sofa } from 'lucide-react';
...
interface SeatMapProps {
  ...
  onEmptySeatClick?: (row: string, col: number) => void;
  isAdmin?: boolean;
  onSeatDrop?: (sourceSeatId: string, targetSeatId: string) => void;
}

export const SeatMap: React.FC<SeatMapProps> = ({ theater, reservations, selectedSeats, onSeatClick, onEmptySeatClick, isAdmin, onSeatDrop }) => {
  ...
  const seatTypesInTheater = new Set((Object.values(theater.seats) as Seat[]).map(s => s.type));
    ...
    <div className="w-full p-4 sm:p-6 md:p-8 bg-white rounded-xl border border-gray-200 shadow-2xl flex justify-center">
      <div className="w-full max-w-full flex flex-col items-center gap-6 md:gap-8">
                ...
                {colIndices.map(col => {
                  ...
                  if (!seat || seat.type === 'empty') {
                    return (
                      <div 
                        key={col} 
                        className="w-full aspect-square border border-transparent hover:border-green-400/50 cursor-pointer transition-colors" 
                        onClick={() => onEmptySeatClick && onEmptySeatClick(row, col)}
                      />
                    );
                  }
                  ...
                  if (seat.type === 'door') {
                    return (
                      <div 
                        key={col} 
                        className="w-full aspect-square flex flex-col items-center justify-center p-0.5 sm:p-1 cursor-pointer hover:opacity-80 transition-opacity z-0"
                        onClick={() => onSeatClick(seat)}
                      >
                        <div className="w-full h-full bg-gray-200 border-2 border-gray-300 rounded-[2px] sm:rounded-md flex items-center justify-center shadow-inner opacity-80 backdrop-blur-sm pointer-events-none">
                          <span className="text-[6px] sm:text-[9px] font-black text-gray-500 tracking-tighter">EXIT</span>
                        </div>
                      </div>
                    );
                  }
                          ...
                          : isSelected
                            ...
                            : seat.type === 'disabled'
                              ...
                              : seat.type === 'sweetbox'
                                ...
                                : seat.type === 'sofa'
                                  ? "bg-[#B45A30]/10 border border-[#B45A30]/50 text-[#B45A30] hover:bg-[#B45A30]/20"
                      ...
                      <div className="absolute bottom-full mb-2 hidden group-hover:block z-50 pointer-events-none">
                        <div className="bg-gray-900 border border-gray-700 text-white text-xs py-1.5 px-3 rounded whitespace-nowrap shadow-xl">
                          ...
                          <span className="text-gray-300 mr-1">
                            {seat.type === 'disabled' ? '(장애인석)' : seat.type === 'sweetbox' ? '(스위트박스)' : seat.type === 'sofa' ? '(소파석)' : ''}
                          </span>
        ...
        <div className="flex flex-wrap gap-4 sm:gap-6 mt-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          {seatTypesInTheater.has('sweetbox') && (
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-md bg-pink-500/10 border border-pink-500/30" />
              <span className="text-xs text-gray-600">스위트박스</span>
            </div>
          )}
          {seatTypesInTheater.has('sofa') && (
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-md bg-[#B45A30]/10 border border-[#B45A30]/50 flex items-center justify-center">
                <Sofa className="w-3 h-3 text-[#B45A30]" />
              </div>
              <span className="text-xs text-gray-600">소파석</span>
            </div>
          )}
          {seatTypesInTheater.has('door') && (
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-md bg-gray-200 border border-gray-300 flex items-center justify-center">
                <span className="text-[6px] font-black text-gray-500">EXIT</span>
              </div>
              <span className="text-xs text-gray-600">출입문</span>
            </div>
          )}
          {seatTypesInTheater.has('disabled') && (
            <div className="flex items-center gap-2">
